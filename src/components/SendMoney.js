...
class SendMoney extends Component {
  ...
  componentDidMount() {
    ...
    .then(personDetail => {
      ...
      this.setState({ senderInfo: personDetail });
    });
  }
  ...
  handleSubmit = e => {
    ...
    const token = "Bearer " + loadedData.token
    const bodyData = {
      "from_account": this.state.senderId,
      "to_account": this.state.receiverId,
    ...
    fetch("http://18.116.9.199:9000/send", requiredOptions)
    .then((response) => response.json())
    .then(res => {
      console.log(res)
      this.setState({ respMessage: res.message });
    });
      ...
      <div class="row">
        <div class="col-md-9 col-lg-7 col-xl-6 mx-auto">
          <div class="bg-white shadow-sm rounded p-3 pt-sm-4 pb-sm-5 px-sm-5 mb-4">
            ...
            <form id="form-send-money" onSubmit={this.handleSubmit}>
              {/* sender wallet id */}
              <div class="mb-3">
                <label for="emailID" class="form-label">Sender Wallet CURRENCY</label>
                ...
                {
                  this.state.senderInfo.map((pr) => (
                    <option key={pr.id} data-icon="currency-flag currency-flag-ars me-1"  data-subtext="Argentine peso" value={pr.wallet_id}>{pr.currency_code}</option>
                  ))
                }
              ...
              {/* receiver wallet id */}
              <div class="mb-3">
                <label for="emailID" class="form-label">Recipient Wallet ID</label>
                <input type="text" value={this.state.receiverId} onChange={this.handleChange} class="form-control" data-bv-field="emailid" id="emailID" name="receiverId" required placeholder="receiver's wallet ID" />
              </div>
              <div class="mb-3">
                <label for="youSend" class="form-label">You Send</label>
                <div class="input-group">
                  <input type="text" class="form-control" name="amount" onChange={this.handleChange} data-bv-field="youSend" id="youSend" placeholder="1000" />
                  
                </div>
              </div>
              <p class="text-muted text-center">Receiver gets <span class="fw-500">{this.state.amount}</span></p>
              <hr />
              <p>Total Fees<span class="float-end">0.0</span></p>
              <hr />
              <p class="text-4 fw-500">Total To Pay<span class="float-end">{this.state.amount}</span></p>
              <p><b>{this.state.respMessage}</b></p>
